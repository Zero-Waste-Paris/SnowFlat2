...
class qa_html_theme extends qa_html_theme_base
{
	...
	public function head_metas()
	{
        ...
        $this->output('<link rel="preload" href="qa-theme/SnowFlat2/fonts/fontello.ttf?70015067" as="font" crossorigin>');
		parent::head_metas();
	}
    ...
    public function head_custom()
    {
      ...
      if(!is_null($postid)) {
          $this->output('<meta property="og:image" content="' . qa_opt('site_url') . 'zwp-plugin-page?qa_id='.$postid.'">');
          $this->output('<meta property="og:image:width" content="1200">');
          $this->output('<meta property="og:image:height" content="630">');
      } else {
           $this->output("<meta property=\"og:image\" content=\"" . qa_opt('site_url') . "MQZD_Assets/LOGO/Logo2.png\" />");
      }

      // Twitter
      $this->output('<meta name="twitter:card" content="summary_large_image">');
      $this->output('<meta name="twitter:creator" content="@ZeroWasteParis">');
      if(!is_null($postid)) {
          $this->output('<meta name="twitter:image" content="' . qa_opt('site_url') . 'zwp-plugin-page?qa_id='.$postid.'">');
          $this->output('<meta name="twitter:title" content="Rendez-vous ici pour voir la réponse !" />');
      } else {
          $this->output('<meta name="twitter:image" content="' . qa_opt('site_url') . 'MQZD_Assets/LOGO/Logo2.png" />');
          $this->output('<meta name="twitter:title" content="' . $this->content['site_title'] . '" />');
      }
    ...
    private function current_title()
    {
		...
		return htmlspecialchars(strlen($pagetitle) ? $pagetitle : $this->content['site_title']);
    }
    ...
    private function canonical_url()
    {
		if (isset($this->content['canonical'])) {
            ...
            return htmlspecialchars(qa_opt('site_url'));
        }
    }
	...
	public function head_script()
	{
		...
		$this->content['script'][] = '<script src="' . $jsUrl . '" defer></script>';
	...
	public function attribution()
	{
		...
		$this->output(
			...
			'Thème Snow par <a href="http://www.q2amarket.com">Q2A Market</a> ' ,
			'</div>',
			'<div class="qa-attribution">',
			'Modifié par un membre de <a href="http://zw.fontainebleau-avon.fr">Zero Waste Fontainebleau-Avon</a>' ,
			'</div>'
		);
		parent::attribution();
	}
	...
	private function qam_user_account()
	{
		...
		$handleBlock = empty($handle) ? '' : '<div class="qam-account-handle">Mon compte</div>';
		$this->output(
